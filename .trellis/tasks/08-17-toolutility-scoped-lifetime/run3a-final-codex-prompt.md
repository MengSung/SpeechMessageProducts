...
**這是本任務的最後一個 Run。** 使用者已決定把終點線畫在 A 類：20 個 B 類呼叫點不遷移，
...
Run 2.5a 之後剩 26 個可執行呼叫點：**A 類 6 個、B 類 20 個**。

20 個 B 類全部卡在同一個原因：`Models/InMemoryDataContextSmallGroup.cs` 把 13 個有狀態的
...
> **原第 7 項（`ViewModels/GalleryViewModel.cs:47`）已改判為 B 類，不在本 Run 範圍。**
> 該檔含三個類別（`GalleryViewModel:23`、`RegisterViewModel:33`、`LineBindingViewModel:42`），
> 持有 Factory 的是 **`LineBindingViewModel`**，而它被 `InMemoryDataContextSmallGroup.cs:1099`
> 以 `SessionId + "_LineBindingViewModel"` 快取 30 分鐘 —— 是 B 類持有者。
> 先前的分類按檔名判斷而誤判，已更正。**A 類 6 個、B 類 20 個。**
...
> 20 個 B 類的持有者被 session 快取 30 分鐘，把 scoped 實例存成欄位就是製造
...
- [ ] 遷移第 2 節 6 個 A 類呼叫點（#1 依 2.1 特別處理）
...
- [ ] **建立後續票草稿** `.trellis/tasks/08-17-toolutility-scoped-lifetime/followup-session-cache.md`，
      內容需含：問題陳述（13 個 session 鍵快取）、受影響的 20 個呼叫點、
...
不准刪 `ToolUtilityFactory`。** `ToolUtilityFactory` 與其 legacy 建構式因 20 個 B 類殘留
...
數量應為 20（若 #1 的無參數多載可刪，則 A 類 6 個全數歸零）。

```bash
grep -rn "ToolUtilityFactory" --include=*.cs SpeechMessageProducts.ChurchReport/Tools/ | grep -vE ":[0-9]+:\s*(//|///)"
```

必須 0 行（A 類集中在 Tools/，已完全遷移）。
**注意：`ViewModels/` 不列入此檢查** —— `LineBindingViewModel` 是 B 類殘留，會且應該留在那裡。
...
- 6 個 A 類呼叫點各自的遷移方式與結果；#1 的無參數多載是刪除還是保留（含理由）
- 20 個 B 類殘留清單（檔案:行號 + 所屬 session 快取 holder）
- `prd.md` 驗收標準的修訂前後對照
- 第 6、7 節每一道指令的**實際輸出原文**，不要摘要
- 後續票的路徑與摘要
- 「等待人工回歸」：登入、會友查詢／編輯、奉獻、影像上傳、LINE 綁定、批次下載、
  QR Code 產生（本 Run 動到 4 個 QrCode utility 與 2 個付款 processor，回歸範圍要涵蓋這兩塊）
