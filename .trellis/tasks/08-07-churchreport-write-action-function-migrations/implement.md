...
- [ ] 讀取本 task 涵蓋 package 的 `.trellis/spec/` index 與 required guideline docs，再開始修改程式。
- [ ] 以 `git status --short` 建立 scoped baseline；保留並排除既有無關 CCG／P7.0 dirty changes，禁止 `git add -A`。

## Phase 1：Activation material

- [x] 建立 `prd.md`、`design.md`、本計畫及 `p7.2-fixture-activation-matrix.json`。
- [ ] 驗證 matrix JSON、source matrix SHA-256、UTF-8 no-BOM、CRLF-only、final CRLF 與 `git diff --check`。
- [ ] 以 task-local `implement.jsonl` 記錄 matrix、P7.1 contract、Data8／ProductClient／ChurchReport source 與 relevant specs；不得把 credential、endpoint、Organization ID 或 fixture ID 寫進 jsonl。
- [ ] 依連續 Goal 授權執行 `task.py start`；它只允許本機測試與實作，尚不允許 CE write，直到 slice A bridge preflight 變為 go。
...
1. 先完成 repository 內 contract／unit tests，再建立 Windows PowerShell 5.1-compatible bridge。它必須 bounded、fail-closed，只允許 `crm91`／Data8／P7.2 marker contact／兩個字串欄位，且只輸出去識別化 JSON。
2. bridge 依序執行 baseline read → sentinel update → read-back reconciliation → baseline restore → restore read-back；任何 ambiguous timeout、owner mismatch、profile mismatch 或 cleanup 失敗都停在 no-go，絕不自動重送不確定 write。
3. 只有 bridge 的 sanitized `outcome=go` 才可寫入 task evidence；不得貼入密碼、token、cookie、endpoint、Organization ID、contact GUID、原始 baseline 或例外內容。
...
- [ ] 執行 focused tests、完整 `SpeechMessage.Dynamics.Tests`、ChurchReport tests、Release build、resource stress／soak、drain／dispose 與 rollback drill。
- [ ] 執行 `git diff --check`、byte-level UTF-8 no-BOM／CRLF-only／final CRLF check，並確認 diff 只含 P7.2-owned files。
