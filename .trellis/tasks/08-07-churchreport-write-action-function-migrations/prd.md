...
將 P7.0 coverage matrix 指派給 `P7.2-write-action-function` 的 24 個 ChurchReport CRM 寫入、Action 與 Function call-site，逐一改為 Data8-first 的具名能力。ChurchReport 必須只透過 ProductClient／Gateway contract 執行經允許的能力；不得暴露通用 CRUD、任意 FetchXML、任意 `OrganizationRequest`、呼叫端自選 CE／Profile／Connector 或直接 CRM SDK 存取。
...
- `sunnyvalechback` 是與正式組織隔離的 CE 9.1 開發 Organization；使用者已允許建立／使用 task-owned 測試會員，但此授權不等於任意寫入、刪除或操作非測試資料。
...
4. 以相同規則完成其餘 P7.2 matrix-required 切片；金融、預約、會員清單、到會與新人流程各自擁有 rollback 邊界，不能與 contact profile 寫入混成同一個 transaction。
...
- 對非 task-owned CE 記錄寫入、刪除或以「寫了再刪」取代已定義的 cleanup。
...
| B. Contact profile / media / aggregate | `memberinfo.contact.update.line.profile`、`memberinfo.contact.count.ungrouped.commitment`、`memberinfo.contact.update.image`、`newperson.contact.update.image` | 4 | 單一 contact；LINE、image 與 aggregate 各有獨立清理策略 | fixture-pending |
...
- [ ] `p7.2-fixture-activation-matrix.json` 覆蓋全部 24 個 P7.2 candidate，並且每個可執行切片都有單一 owner、預設拒絕狀態與不可混用的 rollback 邊界。
