...
- Require lifecycle ownership and cleanup for relevant subscriptions, timers, background tasks, caches, collections, streams, handles, and disposable resources.
- Require targeted tests, stress checks, or profiling when a change creates a credible isolation or memory-retention risk.
- Keep speed and memory-efficiency goals subordinate to security, correctness, cleanup, and maintainability.
- Add the guidance outside the existing CCG-managed block and preserve all unrelated global and repository configuration.

## Acceptance Criteria

- [ ] A concise global policy exists outside the CCG-managed block in `C:\Users\Administrator\.codex\AGENTS.md`.
- [ ] The policy explicitly marks session leakage and memory leaks as zero-tolerance release blockers.
- [ ] The policy covers prevention, review, and risk-based verification rather than only aspirational wording.
- [ ] The policy states that performance optimization cannot trade away isolation, correctness, or deterministic cleanup.
- [ ] Existing managed instructions and unrelated workspace changes remain untouched.
