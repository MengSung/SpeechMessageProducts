...
- [ ] 先在 lifecycle tests 寫 base=false、base-only、both-gates、空白 profile 的 direct factory test；
      確認 red test，然後新增 Package03 metadata predicate/factory，維持 image gate 與其他 gate false。
- [ ] 先寫 service tests：固定 profile/workload/target、defensive copy、結構錯誤、A/B profile isolation、
      cancellation/no retry。確認未實作 service 時為 red。
- [ ] 實作 service/result，僅允許 bounded immutable DTO projection；每筆 validation 完成前不得 publish。

## 3. Controller integration

- [ ] 先寫 source-contract tests：每個 action 在 user/client work 前決定 metadata gate，true path only
      uses typed service plus `RequestAborted`，false path preserves named legacy helpers；generic catches exclude
      cancellation。
- [ ] 以 action-local snapshot coordinator 修改 `SearchDistrictTree`、`LoadGroupMembers`、
...
- [ ] 更新兩份 appsettings，新增 false metadata sub-gate 與繁中 rollback comment。

## 4. 驗證與交付

- [ ] 執行 metadata service/factory/controller focused tests，後執行完整 `ChurchReport.MemberInfo.Tests`、
      `SpeechMessage.Dynamics.Tests`、solution test 和 Release build。
- [ ] 檢查 changed files UTF-8 no BOM/CRLF/final CRLF、`git diff --check`、source-only scope、
      task validation；確認既有不相關 dirty paths 沒被 stage。
- [ ] 限時 CCG dual-model review；若缺額度或逾時，記錄 `雙模型未完成` 和本機驗證結果。
- [ ] 依 Trellis Check、spec update、scope-only commit/archive 完成本 child。保留 parent P7.4 active，
