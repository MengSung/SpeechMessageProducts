...
```mermaid
flowchart LR
    A["P5 Dedicated Gateway 已結案"] --> B["P6 Official Worker Router 擴充點已完成；live evidence pending"]
...
| Official Worker | 已有兩個 identity operation 與 `fee.dedication.retrieve.by.contact.date.range` 的 bounded worker contract；P6.1 Router／Pool／Lease 離線 gate 已通過 | readiness 已 `go`，但 CE 8.2／9.1 Worker 都在 READY 前結束；必須標為 `evidence-pending`，不得宣稱真機可用 |
...
或把 P6.1 離線完成誤報為 Official Worker 已通過真機驗證。
...
P7.1～P7.3 每次只處理一個業務 capability slice，先建立 typed request/response、authorization、bounded response/paging、connector/CE evidence 和故障 cleanup 契約，再在 P7.4 以 capability-specific flag 啟用 consumer。相同 typed capability 必須可由 deployment configuration 選擇 `Embedded + Data8` 或 `DedicatedGateway + Data8`，不得在產品業務碼硬編碼 mode 或 connector。Rollback 必須是關閉該 capability flag、停止新 admission、drain in-flight lease/worker/stream，並回到已核准的 legacy path；不得變更 profile、credential、connector 或把失敗請求改送其他 CE version。
...
P7.5 結案 artifact 必須固定 ProductClient／operation contract 版本、support matrix、deployment package、required profile aliases、sanitized evidence、resource baseline 與 rollback package。P8.0 只接收這組已封存輸入來評估雲端 readiness；Central Gateway composition 必須保留 Data8，第一個 ChurchReport cloud deployment 採 `CentralGateway + Data8`。不得在 P7.0～P7.5 先建立 cloud identity、TLS、DNS 或 Central Gateway traffic。

## 7. 本輪已知缺口與決策

本輪沒有阻礙規劃文件完成的產品決策：使用者已明確要求 P6 擴充點結案後啟動
P7.0→P7.1～P7.5；Lenovo 必須保留 `Embedded + Data8` 與 `DedicatedGateway + Data8`，
之後另以 P8.0～P8.4 將 ChurchReport 以 `CentralGateway + Data8` 部署到雲端。
Official Worker live evidence 仍是誠實的 `evidence-pending`，但不阻塞未選用它的 capability。
尚未可以宣稱的結果是各 capability 的最終 CE support、rollout owner、rollback owner、DTO owner
與 feature gate；這些必須由後續獨立 child task 的具體契約與證據填入，不得在 P7.0 推測或預先啟用。
