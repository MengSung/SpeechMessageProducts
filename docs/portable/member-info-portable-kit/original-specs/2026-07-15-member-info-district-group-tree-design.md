...
| 區名 | `list.new_area_name`（值為「{區牧}牧區」，例：範例牧區） | 若空白，退而用區牧 `new_contact_list_arealeader` 的姓名 ＋「牧區」 |
...
```
▼ 範例牧區   區長：區長甲            （本區 32 人）   ← 第1層（區）：預設展開
    ▶ 範例小組甲   小組長：小組長甲     （8 人）        ← 第2層（小組）：名稱看得到、成員收合
    ▶ 範例小組乙     小組長：小組長乙     （5 人）
    ▶ 範例小組丙    小組長：會友丁     （6 人）
▼ 範例牧區乙   區長：會友丙            （本區 18 人）
    ▶ 範例小組丁     小組長：…          （7 人）
    ▶ 範例小組戊     小組長：…          （11 人）
▶ 無小組                             （120 人）       ← 只有這個預設收合
```
...
```
    ▼ 範例小組甲   小組長：小組長甲     （8 人）
        ...
        │  👤  陳XX 女 1990/8 09xx 基督徒 新店… 小組組員 妻子 小組長甲 │
        └───────────────────────────────────────────────┘
```
...
{
  "districts": [
    { "raceLeaderKey": "...", "areaName": "範例牧區", "raceLeaderName": "區長甲", "memberCount": 32,
      "groups": [ { "listId": "GUID", "groupName": "範例小組甲", "leaderName": "小組長甲", "memberCount": 8 } ] }
  ],
...
- **關係／目標**：拆兩欄。一人多筆時，兩欄以**相同順序**並排（關係：`妻子、門徒`／目標：`小組長甲、小組長乙`）。
