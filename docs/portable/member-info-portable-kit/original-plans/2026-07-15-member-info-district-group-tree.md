...
- 所有指令請在主儲存庫根目錄執行：`<來源儲存庫根目錄>`（分支 `Sunny_5.1.2.TuneMemberView`）。
...
{
    private static SmallGroupDescriptor Grp(string listId, string race, string raceKey, string area = "範例牧區")
    ...
    {
        var groups = new[] { Grp("L1", "區長甲", "R1"), Grp("L2", "區長甲", "R1") };
        ...
        tree.Districts[0].RaceLeaderName.Should().Be("區長甲");
    ...
    {
        var groups = new[] { Grp("L1", "區長甲", "R1", area: "") };
        var tree = DistrictTreeBuilder.Build(groups, new GroupMembershipRow[0], new string[0], false, "church");
...
public class RelationGoalFormatterTests
{
    ...
    {
        var items = new List<(string, string)> { ("妻子", "小組長甲"), ("門徒", "小組長乙") };
        var (rel, goal) = RelationGoalFormatter.Format(items);
        rel.Should().Be("妻子、門徒");
        goal.Should().Be("小組長甲、小組長乙");
    }

    [Fact]
    public void Deduplicates()
    {
        var items = new List<(string, string)> { ("妻子", "小組長甲"), ("妻子", "小組長甲") };
        var (rel, goal) = RelationGoalFormatter.Format(items);
        rel.Should().Be("妻子");
        goal.Should().Be("小組長甲");
    }
    ...
    {
        var items = new List<(string, string)> { ("", ""), ("母親", "小組長甲") };
        var (rel, goal) = RelationGoalFormatter.Format(items);
        rel.Should().Be("母親");
        goal.Should().Be("小組長甲");
    }
