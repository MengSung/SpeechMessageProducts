...
- 只有**傳輸層**故障會將 lease 標記 Faulted：WCF 通道例外、`TimeoutException`、`WebException`、`SocketException` 與 `IOException`。這類例外代表請求可能未送達或回應已損毀，通道狀態不可信；Faulted client 只會 Dispose，不得回池供另一位使用者或另一個 request 重用。
- **商業層 fault 不淘汰連線**：`FaultException`（含 `FaultException<OrganizationServiceFault>`）代表伺服器已完整處理並回傳 SOAP fault，例如欄位不存在、權限不足、驗證規則失敗或伺服器端 plugin 載入失敗。此時通道、token 與安全內容仍健康，淘汰只會造成不必要的重新握手。
- **比對順序不可調換**：`FaultException` 在 WCF 型別階層中是 `CommunicationException` 的子類別。若先比對 `CommunicationException`，所有商業 fault 都會被誤判為連線故障。
- **未知例外採保留連線**：應用程式自身的錯誤不代表通道損毀；出借前的 WhoAmI 健康檢查與下一次操作的傳輸層例外是後備防線。
...
| ⑦ Per-operation 邊界 | `DataverseGateway.Execute`／`Execute<T>` | lease 僅覆蓋單次 CRM 操作，不再由 request 全程占用；只有 `IsConnectionFault()` 判定的傳輸層例外先 `MarkFaulted`，商業層 `FaultException` 與未知應用程式例外則原樣擲回並保留健康連線，finally 一律決定性釋放。 | A5、A7、A10 與 Run C C2/C3；不確定傳輸狀態的 client 不可重用，已完整回覆的商業層 fault 不會造成不必要的連線淘汰。 | — |
...
一次 ChurchReport 真實重現產生 625 次 CRM 操作，其中 7 次以 `ok=false` 結束：2 次為伺服器端 `WeeklyReportPlugIn.dll` 載入失敗，5 次為 `new_start_tracking_date` 欄位探測。7 次均為 `FaultException`，對應的 7 條 lease 全部以 `state=healthy` 歸還，`faulted` 計數恆為 0；後續操作持續沿用同一條連線且皆成功。這證實商業層 fault 已完整回覆而不會污染連線，符合 `IsConnectionFault()` 先排除 `FaultException`、再判定傳輸層例外的實作順序。
