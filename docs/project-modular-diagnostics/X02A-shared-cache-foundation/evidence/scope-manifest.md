...
- `SpeechMessageProducts.ChurchReport/Services/Caching/ISmallGroupCacheManager.cs` and `SmallGroupCacheManager.cs` belong to B03 because they carry small-group-specific cache policy.
- `SpeechMessageProducts.ChurchReport/Controllers/SmallGroupController/SmallGroupController.Cache.cs` is B03 consumer context only.
