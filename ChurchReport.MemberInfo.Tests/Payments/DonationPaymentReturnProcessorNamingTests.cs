...
public sealed class DonationPaymentReturnProcessorNamingTests
{
    ...
    {
        var processorType = Type.GetType($"ChurchReport.Tools.DonationFeePaymentProcessor, {ChurchReportAssemblyName}");
    ...
    {
        var processorType = Type.GetType($"ChurchReport.Tools.RecurringDonationPaymentProcessor, {ChurchReportAssemblyName}");
    ...
    {
        Type.GetType($"ChurchReport.Tools.DonationPaymentResultHelper, {ChurchReportAssemblyName}")
            .Should().NotBeNull("付款結果判斷 helper 應使用中性命名");
        Type.GetType($"ChurchReport.Tools.DonationPaymentDebugLogger, {ChurchReportAssemblyName}")
