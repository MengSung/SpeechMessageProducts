...
public class MemberInfoCommitmentTypeMetadataProviderTests
{
    ...
    public void GetOptions_PreservesConfiguredOrderInsteadOfNumericValueOrder()
    {
        ...
        options.Select(option => option.Value).Should().Equal(100000006, 1, 100000000);
        options.Select(option => option.Order).Should().Equal(0, 1, 2);
    ...
    public void GetOptions_SkipsOptionWithoutValue()
    {
        ...
        metadata.OptionSet.Options.Add(new OptionMetadata(new Label("沒有值", 1028), null));
        using var cache = new MemoryCache(new MemoryCacheOptions());
    ...
    private static PicklistAttributeMetadata Metadata(params (int Value, string Label)[] options)
    {
        ...
        {
            collection.Add(new OptionMetadata(new Label(option.Label, 1028), option.Value));
        }
    ...
    private sealed class RecordingOrganizationService : IOrganizationService
    {
        ...
        public RecordingOrganizationService(AttributeMetadata metadata, bool throwOnExecute = false)
        {
