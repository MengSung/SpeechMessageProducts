// ============================================================================
// AI-繁體中文檔案註解
// 檔案路徑：SpeechMessage.Payments/Providers/Taishin/TaishinCallbackParser.cs
// 所屬區塊：可重用付款核心、付款 provider、ASP.NET Core 整合、workflow 與測試模組。
// 檔案責任：此檔案位於付款相關流程，註解重點在說明 provider 邊界、金流狀態、錯誤處理與不可改變的外部契約。
// 主要型別：class TaishinCallbackParser
// 主要成員：Parse、CopyObjectProperties、AddNormalized、CopyIfPresent、ParseMinorAmount、NormalizeCurrency、GetValue、FirstNonEmpty
// 引用命名空間：System.Globalization、Newtonsoft.Json.Linq、SpeechMessage.Payments.Configuration、SpeechMessage.Payments.Diagnostics、SpeechMessage.Payments.Models
