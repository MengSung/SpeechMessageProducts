// ============================================================================
// 檔案：SpeechMessage.Dynamics.ControlPlane/Runtime/ProfileRoutedOperationExecutor.cs
...
/// <summary>
/// 固定單一 Profile 的中立 Provider。
/// 它把既有固定 Executor 與 Admission Manager 包裝成同一個合併租約契約，讓單一 Profile 路徑
...
internal sealed class FixedProfileExecutionLeaseProvider : IProfileExecutionLeaseProvider
{
    ...
    /// <summary>
    /// 建立不擁有注入資源的固定 Provider；Executor 與 Admission Manager 仍由原本的 Factory／Host 負責回收。
    /// </summary>
    ...
    /// <summary>
    /// 解析固定單一 Profile 路徑的 Admission Plan。空白 Alias 會 fail closed；其他 Alias 只代表既有受控入口，
    /// 不會改變 Executor、Endpoint、Credential 或 Transport，也不會觸發任何外部 I/O。
    /// </summary>
    ...
    /// <summary>
    /// 取得固定 Executor 路徑的 Admission Permit，成功後把 Permit 與借用的 Executor 包裝成單一合併租約。
    /// Provider 不擁有 Executor 或 Admission Manager，只接管本次 Permit；取得失敗時不回傳半完成資源，
        ...
        CancellationToken cancellationToken)
    {
        ...
        return ProfileExecutionLeaseAcquireResult.Success(
            new FixedExecutionLease(_executor, _admissionManager.Plan, admission.Permit));
    }

    /// <summary>
    /// 固定 Executor 路徑的合併租約。沒有 Runtime 引用計數可釋放，因此唯一資源是 Admission Permit；
    ...
    private sealed class FixedExecutionLease : IProfileExecutionLease
    {
        ...
        /// <summary>取得由外層 Host 擁有的固定 Executor；此 Lease 不 Dispose Executor，只限制其使用範圍。</summary>
