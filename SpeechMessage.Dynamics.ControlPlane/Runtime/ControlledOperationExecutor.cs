// ============================================================================
// 檔案：SpeechMessage.Dynamics.ControlPlane/Runtime/ControlledOperationExecutor.cs
// 目的：registry 驗證 -> admission permit -> generation-owned 中立 operation executor。
//
// 保母教學：
...
public sealed class ControlledOperationExecutor : IDynamicsOperationExecutor
{
    ...
    /// <summary>
    /// 建立固定單一 Profile 的中立執行路徑。此建構式把固定 Executor 與 Admission Manager 包裝成合併租約 Provider，
    ...
    public ControlledOperationExecutor(
        ...
        : this(new FixedProfileExecutionLeaseProvider(executor, admissionManager))
    {
    }
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        return ExecutePreparedAsync(prepared, cancellationToken);
    }
        ...
        CancellationToken cancellationToken)
    {
        try
        {
            ...
            await using (acquisition.Lease.ConfigureAwait(false))
            {
                ...
                // 只從 prepared owner 建立新的 SDK-free request；原始 caller request、JSON graph 與
                // HttpContext 早已離開 async state machine。Executor 只能看到核准 alias、operation、
                // server-derived workload、bounded scalar 參數與冪等鍵，不能收到 endpoint 或 Credential。
                return await acquisition.Lease.Executor.ExecuteAsync(
