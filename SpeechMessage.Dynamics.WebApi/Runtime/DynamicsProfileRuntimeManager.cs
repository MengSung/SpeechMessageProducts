// ============================================================================
// 檔案：SpeechMessage.Dynamics.WebApi/Runtime/DynamicsProfileRuntimeManager.cs
...
/// Client／Transport／Token Provider／Handler，Admission Registry 才是同一實體 Organization 容量的唯一共享邊界。
/// </summary>
/// <remarks>
...
public sealed class DynamicsProfileRuntimeManager : IDynamicsProfileRuntimeManager
{
    ...
    /// Client、Token、Handler、CTS 或 Admission Registration；已 Disposed 的 cleanup failure 仍向上回報但不永久卡住 Slot。
    /// </summary>
    ...
    /// <summary>
    /// 在不觸發 Secret、Factory、Token、Admission I/O 或 Transport 的前提下，解析目前 Active Alias 的不可變容量計畫。
    ...
    /// Queue 等待期間不持有 Client／Handler／Runtime 強引用；若 Runtime 交換、不相容或取得失敗，
        ...
        CancellationToken cancellationToken)
    {
        ...
        // replace-and-drain 會被未 dispatch 的工作阻塞，且 Queue 會長期保留舊 Client／Handler Generation。
    ...
    /// 快照只複製 Key、狀態、引用數與 Admission 指標，不把 Client、Token、Credential、Request 或 Runtime 物件交給呼叫端。
    /// </summary>
    ...
    /// 並在方法內確定性 Dispose 候選 Runtime，避免未發布 Handler／Token／Registration 洩漏。
    /// </summary>
        ...
        CancellationToken callerCancellationToken)
    {
        ...
        try
        {
            ...
            if (pendingDraining is not null)
            {
                ...
                // 否則同一 Alias 會同時存在 Active 加兩個 Draining，突破連線與 Token 資源上限。
    ...
    /// 且 Slot 仍指向同一物件時移除強引用。如此 Token／Transport／Admission cleanup failure 仍可被 caller 觀察，
    /// 但已 Disposed 的幽靈 Runtime 不會永久阻塞替換；caller cancellation 或 timeout 留下的未完成 Runtime 則繼續由 Slot 擁有，
    /// 讓後續 Replace 或 Manager Shutdown 能重新接管，而不是遺留不可回收的 Handler、CTS、Lease 或背景工作。
    /// </summary>
    ...
    /// <summary>
    /// 在 Catalog 鎖內只擷取排隊所需的 Admission Manager 與不可變 Plan，不把 Runtime、Client、Token 或 Handler
    ...
    /// 回報所有失敗，避免第一個錯誤阻止其他 Handler、Token Provider、Registration 或 Host Slot 回收。
    /// </summary>
    ...
    private sealed class CombinedExecutionLease : IProfileExecutionLease
    {
        ...
        /// <summary>取得租約生命週期內唯一允許使用的 Client；租約釋放後呼叫端不得保存或再次使用此參考。</summary>
        public IDynamicsWebApiClient Client => _runtimeLease.Client;
