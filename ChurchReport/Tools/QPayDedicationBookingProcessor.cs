...
namespace ChurchReport.Tools
{
    public class QPayDedicationBookingProcessor : Controller, IDisposable
    {
        ...
        private static string GetLineChannelAccessToken()
        {
            try
            {
                ...
                if (!string.IsNullOrEmpty(organization))
                {
                    ...
                    {
                        System.Diagnostics.Trace.WriteLine($"[QPayDedicationBookingProcessor] LINE Token loaded for organization: {organization}");
                        return token;
                    }
                }
                ...
                {
                    System.Diagnostics.Trace.WriteLine("[QPayDedicationBookingProcessor] 警告: LINE Channel Access Token 未設定");
                }
            ...
            {
                System.Diagnostics.Trace.WriteLine($"[QPayDedicationBookingProcessor] 錯誤: 讀取 LINE Token 設定失敗 - {ex.Message}");
                return string.Empty;
            }
        }
        ...
        #endregion
        #region 主程式
        public ActionResult QPayDedicationBookingProcessorReturnUrl(string ShopNo, String PayToken, QPayWorkflowPaymentResult paymentResult, string correlationId = "", string requestContext = "")
        {
            try
            {
        ...
        #endregion
        #region 建立收費單
        public Guid CreateFee(Entity aContact, Entity aDedicationBookingEntity, QPayWorkflowPaymentResult paymentResult, String Description)
        {
            try
            {
                #region 建立收費單
        ...
        public void SetFeeParameter(Entity aContact, Entity aFeeToCreated, Entity aDedicationBookingEntity, QPayWorkflowPaymentResult paymentResult, String Description)
        {
            try
            {
                #region 建立收費單所需要的參數
                ...
                if (QPayPaymentResultHelper.IsPaymentSuccess(paymentResult))
                {
                    // 信用卡付款結果成功
                    // 收費單付款狀態，是信用卡已繳費
