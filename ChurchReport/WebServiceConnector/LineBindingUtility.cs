// ============================================================================
// AI-繁體中文檔案註解
// 檔案路徑：ChurchReport/WebServiceConnector/LineBindingUtility.cs
// 所屬區塊：ChurchReport 主網站與後台應用程式，承載控制器、模型、CRM 整合、付款流程、LINE 通知與產品層商業規則。
// 檔案責任：此檔案位於服務或工具層，註解重點在說明共用責任、外部依賴、錯誤傳遞與呼叫端應遵守的前置條件。
// 主要型別：class LineBindingUtility
// 主要成員：VerifyContact、RegisterContact、ProcessNotYetBinding、ProcessNotFindRegister、IsBindingAlreadyOrNot、IsBindingAlready、ProcessEachContactWithSameName、ProcessEachSameNameContactButNoLineId、CopyLineInfomation、FindEmptyMobileContact
// 引用命名空間：System、System.Collections.Generic、System.Linq、System.Threading.Tasks、ToolUtility、ChurchReport.Models.CrmTransmitModule、ChurchReport.ViewModel、Microsoft.Xrm.Sdk
    ...
    public class LineBindingUtility
    {
        ...
        private void CopyVistorCardInfo(LineBindingViewModel aLineBindingViewModel, ref Entity ToContact)
        {
            try
            {
                ...
                if (aLineBindingViewModel.Status != null && !string.IsNullOrWhiteSpace(aLineBindingViewModel.Status))
                {
                    try
                    {
                        ...
                        int spiritualIdentityValue = optionSetService.GetOptionSetValue(
                            ...
                            aLineBindingViewModel.Status
                        );

                        this.m_ToolUtilityClass.SetOptionSetAttribute(ref ToContact, "new_spiriitual_identity", spiritualIdentityValue);

                        System.Diagnostics.Debug.WriteLine($"[LineBindingUtility.CopyVistorCardInfo] 成功設定信仰狀態: {aLineBindingViewModel.Status} -> {spiritualIdentityValue}");
                    }
