// ============================================================================
// AI-繁體中文檔案註解
// 檔案路徑：ChurchReport/WebServiceConnector/NewPerson.cs
// 所屬區塊：ChurchReport 主網站與後台應用程式，承載控制器、模型、CRM 整合、付款流程、LINE 通知與產品層商業規則。
// 檔案責任：此檔案位於服務或工具層，註解重點在說明共用責任、外部依賴、錯誤傳遞與呼叫端應遵守的前置條件。
// 主要型別：class NewPerson
// 主要成員：CreateNewContactFromView、FilterNewContact、SearchContactByMobilePhone、AddNewContactToList、DoesContactAlreadyInASmallGroup、CreateNewContact、SetupNewContactParameter、ConvertIndexToDisplayContext、GetRelatedList、ConnectNewContactInMemberList
...
namespace ChurchReport.WebServiceConnector
{
    public class NewPerson
    {
        ...
        public String FilterNewContact(AccountPasswordData aAccountPasswordData, ref NewContact aNewContact)
        {
            try
            {
                // 依據姓名及手機號碼過濾，看新人是否已存在
        ...
        public Entity SearchContactByMobilePhone(ref NewContact aNewContact)
        {
            #region 依據姓名，看新人是否已存在

            // 同名同姓
            // 行動電話或是住家電話一樣，就認定是資料庫已經有這個人
            EntityCollection ContactEntityCollection = m_ToolUtilityClass.RetrieveContactEntityByFullNameCollection(aNewContact.Name);

            foreach (Entity ContactEntity in ContactEntityCollection.Entities)
            {
                String aExistContactMobilePhone = DigitsOnly.Replace(this.m_ToolUtilityClass.GetEntityStringAttribute(ContactEntity, "mobilephone"), "");

                aExistContactMobilePhone = DigitsOnly.Replace(aExistContactMobilePhone, "");

                aNewContact.MobilePhone = DigitsOnly.Replace(aNewContact.MobilePhone, "");

                if (aNewContact.MobilePhone == aExistContactMobilePhone)
                {
                    // 行動電話一樣
        ...
        public String AddNewContactToList(AccountPasswordData aAccountPasswordData, ref NewContact aNewContact, Entity aExistContact)
        {
            try
            {
                #region // 新增的新人已經存在在資料庫中了
                // 判斷是否已經在其他小組中
                String SmallGroupStatus = "";
                Entity aExistList = DoesContactAlreadyInASmallGroup(aExistContact, ref SmallGroupStatus);
                ...
                else
                {
                    ...
                    else
                    {
                        ...
                        //String AddedGroupName = m_ToolUtilityClass.GetEntityStringAttribute(ListEntity, "listname");
                        String DestinyGroupName = this.m_ToolUtilityClass.GetEntityStringAttribute(aExistList, "listname");
        ...
        public Entity DoesContactAlreadyInASmallGroup(Entity aExistContact, ref String SmallGroupStatus)
        {
            try
            {
                ...
                foreach (Entity ListEntity in aLists.Entities)
                {
                    ...
                    {
                        #region// 這個名單是需要被點名的 ，已經在其他小組中

                        //String LoginContactFullName = this.m_ToolUtilityClass.GetEntityStringAttribute(this.m_ContactEntity, "fullname");
                        //String ExistContactFullName = this.m_ToolUtilityClass.GetEntityStringAttribute(aExistContact, "fullname");

                        ////String AddedGroupName = m_ToolUtilityClass.GetEntityStringAttribute(ListEntity, "listname");
                        //String DestinyGroupName = m_ToolUtilityClass.GetEntityStringAttribute(ListEntity, "listname");

                        //String Result = LoginContactFullName + " 想要加入 " + ExistContactFullName + " 到 " + AddedGroupName + "小組中" + "但是" + ExistContactFullName + "已經在 " + DestinyGroupName + " 小組了!"

                        ////傳送LINE 訊息關於加入新人的結果給權柄
                        //this.m_LineNotifyUtility.SendAddNewPersonResultLine(Result, ListEntity);
            ...
            try
            {
                ...
                if (GroupWeeklyReportEntityCollection.Entities.Count == 1)
                {
                    ...
                    if (PresentRecordEntityCollection.Entities.Count == 0)
                    {
                        ...
                        // Note: aNewContact variable removed as it doesn't exist in this context
                    }
                }
                #endregion
            }
