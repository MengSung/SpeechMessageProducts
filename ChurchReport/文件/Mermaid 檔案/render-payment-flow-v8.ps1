...
if (-not ('PaymentFlowPngWriterV8' -as [type])) {
...
public static class PaymentFlowPngWriterV8
{
...
$outputPath = [System.IO.Path]::Combine($scriptDir, 'payment-flow-complete-v8.png')
$width = 2600
$height = 5200
...
function Draw-Step($key, $y, $tag, $title, $body, $fillHex, $strokeHex, $rightNote = '') {
    $script:steps[$key] = [pscustomobject]@{ Y = $y; X = 560; W = 1200; H = 150 }

    Draw-Rect 150 ($y + 28) 330 92 $fillHex $strokeHex 3
    Draw-Text $tag $tagFont $textBrush 168 ($y + 44) 294 60 $centerFormat

    Draw-Rect 560 $y 1200 150 $fillHex $strokeHex 3
    Draw-Text $title $nodeTitleFont $textBrush 586 ($y + 18) 1148 36
    Draw-Text $body $nodeBodyFont $textBrush 586 ($y + 62) 1148 76

    if (-not [string]::IsNullOrWhiteSpace($rightNote)) {
        Draw-Rect 1850 ($y + 10) 560 130 '#f8fafc' '#cbd5e1' 2
        Draw-Text $rightNote $smallFont $mutedBrush 1870 ($y + 24) 520 96
    }
}
...
Draw-Text (T '5a6M5pW06YeR5rWB5ZG85Y+r5rWB56iLIFY4IC0g5riF5pmw5pmC6ZaT6Lu454mI') $titleFont $textBrush 560 30 1480 60 $centerFormat
Draw-Text (T '5omA5pyJ566t6aCt5Y+q5rK/5Lit5aSu5b6A5LiL6LWw77yb5q+P5LiA5q2l5bem5YG05qiZ56S66LKs5Lu75bGk77yM5Y+z5YG06Kqq5piO5qC45b+D6YKK55WM6IiH5Y+v6YeN55So5L2N572u77yM6YG/5YWN57ea5qKd6YGu5L2P5paH5a2X44CC') $subtitleFont $mutedBrush 230 94 2140 62 $centerFormat
...
Draw-LegendItem 520 245 (T 'Q2h1cmNoUmVwb3J0IOeUouWTgeWxpA==') '#fff7ed' '#c2410c'
Draw-LegendItem 890 245 (T 'QVNQLk5FVCDlhbHnlKjmqYvmjqXlsaQ=') '#ecfeff' '#0891b2'
Draw-LegendItem 1260 245 (T 'U3BlZWNoTWVzc2FnZS5QYXltZW50cyDntJTph5HmtYHmoLjlv4M=') '#eef2ff' '#4f46e5'
Draw-LegendItem 1630 245 (T '56ys5LiJ5pa56YeR5rWBIFByb3ZpZGVy') '#f0fdf4' '#16a34a'
Draw-LegendItem 2000 245 (T '55Si5ZOB5LuY5qy+5b6M5bel5L2c5rWB') '#f8fafc' '#64748b'
...
Draw-BoundaryCard 940 4410 (T 'SG9zdCDpgornlYw=') (T 'U3BlZWNoTWVzc2FnZS5QYXltZW50cy5Bc3BOZXRDb3Jl77yaSHR0cFJlcXVlc3Qg5bCN5oeJ44CBQUNLIOi9iSBBU1AuTkVUIOWbnuaHieOAgURJIOi8lOWKqeOAgg==') '#ecfeff' '#0891b2'
Draw-BoundaryCard 1730 4410 (T '55Si5ZOB6YKK55WM') (T 'Q2h1cmNoUmVwb3J0IOiIh+acquS+hueUouWTge+8mkNvbnRyb2xsZXLjgIHpoIHpnaLjgIFDUk0vRELjgIFMSU5FL+mAmuefpeOAgeWOu+mHjeOAgeWVhualrea1geeoi+OAgg==') '#fff7ed' '#c2410c'
Draw-BoundaryCard 545 4630 (T 'TGluZVBheSDoqqrmmI4=') (T 'TGluZVBheSDnm67liY3ku43mmK/njajnq4vlsIjmoYjvvIzmspLmnInntI3lhaXmnKzmrKEgU2lub3BhYy9RUGF544CBTXlQYXnjgIFUYWlzaGluL1RTUEcg55qE5qC45b+D5oq96Zui44CC') '#f8fafc' '#64748b'
...
[PaymentFlowPngWriterV8]::Save($bitmap, $outputPath)
