    ...
    public class QpayManager : Controller
    {
        ...
        public async Task<IActionResult> QueryKeyInDedication(QpayModel QpayModel)
        {
            try
            {
                ...
                EntityCollection DedicationContacts = this.m_ToolUtilityClass.QueryDediccationContatsByFetchXml(DedicationNumber, FullName, "未填手機號碼", Mobile, NationId, LastSixDigit);
                ...
                if (DedicationContacts.Entities.Count == 1)
                {
                    ...
                    String Ntbt = this.m_ToolUtilityClass.GetEntityBoolAttribute(DedicationContacts.Entities[0], "new_ntbt_ornot") == true ? "是" : "否";
                    String PublishDonation = this.m_ToolUtilityClass.GetEntityBoolAttribute(DedicationContacts.Entities[0], "new_donation_no_publish") == true ? "是" : "否";
                    ...
                    return Json(new { status = "1", clicktype = "查詢", DedicationNumber = DedicationNumber, NationId = NationId, FullName = FullName, Mobile = PhoneNumber, LastSixDigit = LastSixDigit, message = DedicationResult, DedicationResult = DedicationResult, ChurchName = ChurchName, Ntbt  = Ntbt, PublishDonation = PublishDonation });
                }
                else if (DedicationContacts.Entities.Count > 1)
                {
                    ...
                    foreach (Entity aContact in DedicationContacts.Entities)
                    {
                        ...
                        aSameNameElement.Ntbt = this.m_ToolUtilityClass.GetEntityBoolAttribute(aContact, "new_ntbt_ornot") == true ? "是" : "否";
                        aSameNameElement.PublishDonation = this.m_ToolUtilityClass.GetEntityBoolAttribute(aContact, "new_donation_no_publish") == true ? "是" : "否";
        ...
        public async Task<IActionResult> UpdateKeyInDedication(QpayModel QpayModel)
        {
            try
            {
                if ( QpayModel.Amount != null && QpayModel.Amount > 0  && QpayModel.Category != "無息借款")
                {
                    // 一般正常奉獻
                ...
                else if (QpayModel.Amount != null && QpayModel.Category == "無息借款")
                {
                    // 無息借款
                    String DedicationResult = await m_QPayProcessor.SaveKeyInDedication(QpayModel);
                ...
                else if (QpayModel.Amount != null && QpayModel.Amount <= 0 && QpayModel.Category != "無息借款")
                {
                    return Json(new { status = "3", clicktype = "上傳", DedicationResult = QpayModel.Category + "奉獻金額不可小於等於0" });
                }
                else
                {
                    return Json(new { status = "3", clicktype = "上傳", DedicationResult = "未輸入奉獻金額" });
                }
            }
            catch (System.Exception e)
            {
        ...
        public QpayModel SetDedicationFeeList(Entity LineLoginContact)
        {
            try
            {
                ...
                foreach (Entity aDedicationFeeEntity in aDedicationFeeEntityCollection.Entities)
                {
                    ...
                    aDedicationFee.Category = ConvertToCategory(aDedicationFeeEntity);
                    if (aDedicationFee.Category != "無息借款")
                    {
                        // 一般奉獻
                        aDedicationFee.Amount = Convert.ToInt32(this.m_ToolUtilityClass.GetEntityMoneyAttribute(aDedicationFeeEntity, "new_fee_really_paid").Value);
                        m_QpayModel.TotalAmount += aDedicationFee.Amount;
                    ...
                    else
                    {
                        // 無息借款
                        aDedicationFee.Amount = Convert.ToInt32(this.m_ToolUtilityClass.GetEntityMoneyAttribute(aDedicationFeeEntity, "new_fee_really_paid").Value);
                        m_QpayModel.TotalBorrowedAmount += -aDedicationFee.Amount;
                    }
        ...
        public QpayModel SetQpayModel(Entity aContact)
        {
            try
            {
                ...
                // 教會職稱是否是會計
                m_QpayModel.IsAOfficeWorker = this.m_ToolUtilityClass.GetEntityStringAttribute(ref aContact, "new_church_jobtitle").Contains("會計" )? true : false;
        ...
        public async Task<IActionResult> CreateContact(string FullName)
        {
            ...
            catch (System.Exception e)
            {
                ...
                aLineMessagingProcessorClass.SendMessage("U7638e4ed509708a3573ba6d69970583d", "南崁基督長老教會 : 註冊錯誤 => " + ErrorString);
