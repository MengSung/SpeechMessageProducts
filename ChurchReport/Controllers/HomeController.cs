...
namespace ChurchReport.Controllers
{
    public class HomeController : BaseChurchController
    {
        ...
        [Route("/Home/PollQrCodeView/{QrCodeViewPatameter}")]
        public IActionResult PollQrCodeView(string QrCodeViewPatameter = null, string QrCodeId = null)
        {
            try
            {
                var model = new PollModel
                {
                    DisplayGrowFlag = false,
                    DisplayDecipleFlag = false,
                    DisplayLeaderFlag = false
                };

                // 根據 QrCodeId 設定顯示的表單區塊
                if (!string.IsNullOrEmpty(QrCodeId))
                {
                    InMemoryContext.ListManager.QrCodeId = QrCodeId;
                    // TODO: 根據 QrCodeId 從資料庫查詢課程類型，設定對應的 Flag
                }
        ...
        /// <summary>
        /// Poll QR Code 取得 Line ID
        /// </summary>
        [HttpPost]
        [Route("/Home/PollQrCodeGetLineId")]
            ...
            string ViewType)
        {
            try
            {
                ...
                var qrCodeUtility = new QrCodeUtility();
                string userName = "";
                string onboardType = "";
                string classIndex = "";
                string className = "";
            ...
            {
                return Json(new { result = $"錯誤: {e.Message}" });
            }
        }
        ...
        [HttpPost]
        [Route("/Home/SavePoll")]
        public async Task<IActionResult> SavePoll(PollModel model)
        {
            try
            {
                if (model == null)
                {
                    return Json(new { status = 0, message = "表單資料不完整" });
                }

                // TODO: 實作儲存問卷資料到 CRM 的邏輯
                // 範例：
                // var service = GetCrmService();
                // var entity = new Entity("your_poll_entity");
                // entity["field_name"] = model.PropertyValue;
            ...
            {
                return Json(new { status = 0, message = $"儲存失敗: {e.Message}" });
            }
        }
