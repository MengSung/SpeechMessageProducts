...
// 3. 只有 registry 的二十四列可帶 response policy；metadata 只能宣告 OptionSet pure-value branch，
...
public sealed class OperationRegistryAgreementTests
{
    ...
    /// <summary>
    /// matrix 只允許目前 registry 所擁有的二十四列宣告 response policy；這防止尚未投產的 call site 偽裝成
    ...
    public void Matrix_response_policy_is_present_for_exactly_current_registry_rows()
    {
        ...
        responsePolicyRows.Should().HaveCount(24);
    ...
    /// <summary>
    /// schema 必須宣告封閉 discriminator、有限整數範圍及只允許二十四個 registry rows 填入政策的條件，否則
