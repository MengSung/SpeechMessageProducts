// ============================================================================
// AI-繁體中文檔案註解
// 檔案路徑：ChurchReport/WebServiceConnector/PersonalInfomatioManager.cs
// 所屬區塊：ChurchReport 主網站與後台應用程式，承載控制器、模型、CRM 整合、付款流程、LINE 通知與產品層商業規則。
// 檔案責任：此檔案位於服務或工具層，註解重點在說明共用責任、外部依賴、錯誤傳遞與呼叫端應遵守的前置條件。
// 主要型別：class PersonalInfomatioManager
// 主要成員：CreateNewContact、SetupNewContactParameter、GetRelatedList、ConnectNewContactInMemberList、CreateNewContactPresentRecord、SetPersonalInfomationViewModel、ConvertIndexToIdentity、ConvertIndexToSpiritualIdentity、ConvertIndexToDisplayContext、UpdatePersonalInfomationViewModel
...
namespace ChurchReport.WebServiceConnector
{
    public class PersonalInfomatioManager
    {
        ...
        public void UpdatePersonalInfomationViewModel(PersonalInfomationViewModel aPersonalInfomationViewModel, Entity aContactEntity )
        {
            ...
            // 設定婚姻狀態 - 改為動態從 OptionSet 查詢
            if (aPersonalInfomationViewModel.MerrageState != null && !string.IsNullOrWhiteSpace(aPersonalInfomationViewModel.MerrageState))
            {
                try
                {
                    // ? 使用 OptionSetMetadataService 動態反向查詢 (文字 -> 數值)
                    ...
                    int familyStatusValue = optionSetService.GetOptionSetValue(
                        "contact",
                        "familystatuscode",
                        aPersonalInfomationViewModel.MerrageState
                    );

                    this.m_ToolUtilityClass.SetOptionSetAttribute(ref aContactEntity, "familystatuscode", familyStatusValue);

                    System.Diagnostics.Debug.WriteLine($"[PersonalInfomatioManager.SetupNewContactParameter] 成功設定婚姻狀態: {aPersonalInfomationViewModel.MerrageState} -> {familyStatusValue}");
                }
                catch (KeyNotFoundException)
                {
                    // 找不到對應的選項，使用備用硬編碼邏輯 => 未知 100000001
            ...
            // ✅ 設定信仰狀態 - 改為動態從 OptionSet 查詢
            if (aPersonalInfomationViewModel.Status != null && !string.IsNullOrWhiteSpace(aPersonalInfomationViewModel.Status))
            {
                try
                {
                    // ✅ 使用 OptionSetMetadataService 動態反向查詢 (文字 -> 數值)
                    ...
                    int spiritualIdentityValue = optionSetService.GetOptionSetValue(
                        "contact",
                        "new_spiriitual_identity",
                        aPersonalInfomationViewModel.Status
                    );

                    this.m_ToolUtilityClass.SetOptionSetAttribute(ref aContactEntity, "new_spiriitual_identity", spiritualIdentityValue);

                    System.Diagnostics.Debug.WriteLine($"[LineBindingUtility.CopyVistorCardInfo] 成功設定信仰狀態: {aPersonalInfomationViewModel.Status} -> {spiritualIdentityValue}");
                }
                catch (KeyNotFoundException)
                {
                    // 找不到對應的選項，使用備用硬編碼邏輯 = >-未知- 100000004
                ...
                if (VerifyNewComerIdentity(aContact))
                {
                    ...
                    if (aIdentityNumber == 100000004)
                    {
                        ...
                        if (aStartTracking != "")
                        {
                            ...
                            #region 先根據日期尋找當週主日日期
                            // 保留既有以目前日期判斷主日的流程，但改成集中式設定驅動的計算方式。
                            DateTime aSunday = ChurchReport.Services.SundayCalculator.CalculateSunday(
                                DateTime.Now,
                                ChurchReport.Services.WeeklyScheduleProvider.FirstDayOfWeek);
                            #endregion
