// ============================================================================
// AI-繁體中文檔案註解
// 檔案路徑：ChurchReport/Controllers/HomeController.cs
// 所屬區塊：ChurchReport 主網站與後台應用程式，承載控制器、模型、CRM 整合、付款流程、LINE 通知與產品層商業規則。
// 檔案責任：此檔案位於控制器層，註解重點在說明 HTTP 入口、產品流程邊界、輸入輸出與外部副作用。
// 主要型別：class HomeController
// 主要成員：LoginRedirect、ProcessLoginRedirect、IntegrateViewRedirect、MultiGroupViewRedirect、NewPersonFollowUpViewRedirect、PersonalReportRedirect、PersonalInfomationViewRedirect、DonationPaymentViewRedirect、ChurchRootRedirect、EquipmentViewRedirect
// 引用命名空間：ChurchReport.Models、ChurchReport.Tools、ChurchReport.ViewModel、ChurchReport.ViewModels、ChurchReport.WebServiceConnector、DevExtreme.AspNet.Data、DevExtreme.AspNet.Mvc、LineMessagingProcessor
...
namespace ChurchReport.Controllers
{
    public class HomeController : BaseChurchController
    {
        ...
        // 註：/Home/DediationLineLoginView 已改由 DedicationController.DediationLineLoginView 直接就地渲染。
        // LIFF Endpoint 如果登錄 /Home/...，不可再 302 轉到 /Dedication/...，
        ...
        public async Task<IActionResult> SetupUserLineIdRedirect(
            string UserLineId,
            string GroupId,
            string RoomId,
            string ViewType,
            string IdToken)
        {
            ...
            {
                // ? 使用 await 調用非同步方法
                return await dedicationController.SetupUserLineId(UserLineId, GroupId, RoomId, ViewType, IdToken);
            }
        }
