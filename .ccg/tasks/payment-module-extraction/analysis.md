...
- Task 8 rewrote `TSPGController` as a thin ChurchReport adapter over `IPaymentGateway` for callback, create, and query routes. The controller no longer owns TSPG request models, callback parsing, status mapping, hash verification, or direct `TspgToolkit` calls.
- `TspgToolkit`, `TspgToolkitWrapper`, `TSPGModels`, `TSPGStoreOrder`, and `TSPGWebhookHandler` remain temporarily because `QPayProcessor` still builds TSPG orders through legacy `IPayment`/provider models. Their removal is deferred to the QPay/QPayProcessor conversion slice so the solution stays buildable between provider migrations.
