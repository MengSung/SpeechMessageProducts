...
<TASK>
# CCG reviewer Task: fix-dual-model-operation-postfix-review

## Repository
D:\音訊科技產品\系統平台\SpeechMessageProducts

## Request
# Review request: fix CCG dual-model operation after quota-detection narrowing
...
index 86779b6c..65e64c90 100644
...
+    return ($Text -match "(?i)(you'?ve hit your session limit|you'?ve reached your .* limit|fable 5 limit|session limit|rate limit|rate_limit|quota exceeded|insufficient_quota|resource_exhausted|usage limit|http\s*429|\b429\b|insufficient balance|balance insufficient|billing|payment required|\u4f59\u989d\u4e0d\u8db3|\u9918\u984d\u4e0d\u8db3|\u4f59\u989d\u4e0d\u591f)")
 }
 
 function Test-BackendQuotaBlocked {
@@ -265,6 +272,59 @@ function Test-BackendProducedOutput {
...
@@ -298,6 +358,39 @@ function Invoke-ClaudeDirectQuotaProbe {
     }
 }
...
+        "C:\Users\Administrator\.claude\bin\gemini.cmd"
+    )
+
+    $geminiPath = $geminiCandidates | Where-Object { Test-Path -LiteralPath $_ } | Select-Object -First 1
+    if (-not $geminiPath) {
+        return [pscustomobject]@{
+            Ran = $false
+            QuotaBlocked = $false
...
@@ -431,6 +524,18 @@ for ($attempt = 1; $attempt -le $MaxAttempts; $attempt++) {
         $diagnostic = $null
...
@@ -445,6 +550,10 @@ for ($attempt = 1; $attempt -le $MaxAttempts; $attempt++) {
...
+        }
         ...
         $attemptRecord.backends += [ordered]@{
             backend = $backend
@@ -452,6 +561,7 @@ for ($attempt = 1; $attempt -le $MaxAttempts; $attempt++) {
...
index f0679465..b432bc0f 100644
...
+    $quotaBlockedPattern = "(?i)(you'?ve hit your session limit|you'?ve reached your .* limit|fable 5 limit|session limit|rate limit|rate_limit|quota exceeded|insufficient_quota|resource_exhausted|usage limit|http\s*429|\b429\b|insufficient balance|balance insufficient|billing|payment required|\u4f59\u989d\u4e0d\u8db3|\u9918\u984d\u4e0d\u8db3|\u4f59\u989d\u4e0d\u591f)"
