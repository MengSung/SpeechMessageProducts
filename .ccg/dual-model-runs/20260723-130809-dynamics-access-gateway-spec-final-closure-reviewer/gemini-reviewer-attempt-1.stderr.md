...
<TASK>
# CCG reviewer Task: dynamics-access-gateway-spec-final-closure
...
- define versioned, length-prefixed canonical tuple encoding for all composite
  runtime/admission/idempotency keys, and define safe rolling handoff: a
  terminating replica reserves its slot through drain, then atomically releases
  it; a transient renewal RPC failure is distinguished from lease rejection or
  TTL expiry.
...
- use one canonical-organization-keyed `OrganizationAdmissions` map (rather than
  ...
  pre-dispatch capacity reservation, alert/high-water/hard-quota behavior, and
  no unbounded in-memory audit retry queue.
...
- If the task cannot be completed, explain the exact blocker.
  PID: 10940
  Log: C:\Users\Administrator\AppData\Local\Temp\codeagent-wrapper-10940.log
