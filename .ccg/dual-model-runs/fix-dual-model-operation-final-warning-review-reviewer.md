# CCG reviewer Task: fix-dual-model-operation-final-warning-review

## Repository
D:\音訊科技產品\系統平台\SpeechMessageProducts

## Request
# Final review request: CCG dual-model runner after warning fixes
...
- Avoiding false degraded fallback for generic 403, generic billing text, and output mismatches.
...
index 86779b6c..d7ded5d8 100644
...
@@ -227,7 +234,7 @@ function Test-BackendQuotaBlocked {
         return $false
     }
 
-    $errorText = (($Result.StdErr + "`n" + $Diagnostic) -replace "`r", "")
+    $errorText = (($Result.StdOut + "`n" + $Result.StdErr + "`n" + $Diagnostic) -replace "`r", "")
...
index f0679465..482563b7 100644
...
@@ -165,18 +191,49 @@ OUTPUT: one line only
                 -WorkingDirectory $RepositoryPath
...
@@ -195,6 +252,9 @@ New-DirectoryIfMissing -Path $resolvedOutputDirectory
 $env:GEMINI_CLI_TRUST_WORKSPACE = "true"
 $env:CODEAGENT_LITE_MODE = "true"
 $env:PYTHONIOENCODING = "utf-8"
+if ([string]::IsNullOrWhiteSpace($env:CLAUDE_MODEL)) {
