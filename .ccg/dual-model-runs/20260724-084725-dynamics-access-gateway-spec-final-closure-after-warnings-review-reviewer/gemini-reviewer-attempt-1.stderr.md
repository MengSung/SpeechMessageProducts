...
<TASK>
# CCG reviewer Task: dynamics-access-gateway-spec-final-closure-after-warnings-review
...
- prohibit any runtime construction of an admission budget, queue/semaphore,
  permit key, or lease namespace from
  `tuple(deploymentEnvironment, expectedOrganizationId)`; use only the
  validated canonical capacity entry and its separately named admission/lease
  namespaces.
- require server-owned FetchXML/OData builders to declare parameter encoding
  contexts, use XML/OData/URI/multipart context-specific encoders, and reject
  any template/parameter combination that can change XML/OData structure,
  entity/attribute/operator names, URL authority, headers, or multipart
  boundaries.
- require the coverage matrix to be a CI completeness gate for every migrated
  call site, including v8.2/v9.1 evidence, parameter encoding contexts, audit
  class, temporary-legacy owner, and removal deadline; verify rollout stage
  names map unambiguously to execution-plan phases.
- require Visual Studio Embedded fake-profile testing to use a separate
  non-production trust anchor (approved local development registry or signed
  Development manifest), never unsigned local JSON or a production trust
  anchor; unavailable/invalid/expired development trust remains NotReady.
- require the machine-readable coverage matrix to be checked automatically
  against the generated operation registry for capability ID, parameter names,
  encoding contexts, version evidence, and audit/idempotency classification.
- keep compression disabled by default and allow it only after profile-gated
  real-target throughput/CPU/p95 evidence plus bounded streaming decompression,
  decompressed-size/expansion-ratio caps, finite encoding-chain policy, and
  malformed/stacked-content tests.
...
- If the task cannot be completed, explain the exact blocker.
  PID: 23704
  Log: C:\Users\Administrator\AppData\Local\Temp\codeagent-wrapper-23704.log
