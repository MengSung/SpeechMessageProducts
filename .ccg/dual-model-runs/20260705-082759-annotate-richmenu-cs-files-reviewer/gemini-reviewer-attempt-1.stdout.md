...
Git: branch Jesus_5.1.7.WorktreeRichMenuAddComment; dirty 84 paths.
Current task: none.
...
- If the task cannot be completed, explain the exact blocker.Trellis SessionStart 整合環境已載入。

本任務為一次性的程式碼變更審查需求（`/ccg:review`），屬於唯讀的 UI 與 Code Review 角色。依據 Trellis 工作流規範，此 turn 屬於「小型的單次諮詢與審查任務」，不涉及任何檔案變更或專案持久性狀態修改，因此無需建立 Trellis 工作任務。

以下為針對目前 `git diff` 中 RichMenu 相關 C# 註解變更的審查報告：

# VALIDATION REPORT
=================
User Experience: 20/20 - 註解詳細說明了產品層 menu key (`legacy-auth`) 與 LINE 平台底層 `richMenuId` 的架構隔離，有助於未來開發者維護與 UX 切換邏輯理解。
Visual Consistency: 20/20 - 不涉及 UI 圖稿或 CSS 變更，但對於 `ImagemapSize` 等版面與點擊座標的註解相當精確。
Accessibility: 20/20 - 對 `DateTimePickerTemplateAction`、`MessageTemplateAction` 等 action label 補上了 LINE iOS 8.2.0+ 的可及性（accessibility）語音朗讀說明，有利於 a11y 規格追蹤。
Performance: 20/20 - 說明了 `InMemoryLineRichMenuIdCache` 鎖定與 snapshot 取代設計，以及避免重複向 LINE 請求的快取機制，效能設計意圖清晰。
Browser Compatibility: 20/20 - 註解中詳細提及了不同 LINE App 版本（如 iOS 8.2.0+, Android 8.11.0+）的相容性邊界。
...
### 1. 行為變更評估 (Behavioral Changes)
- **Info**: 經逐行比對 `git diff`，所有變更皆為 C# 註解（`///` 與 `//`）的加入與微調、或純空格換行調整（`\r\n`），完全**沒有修改任何實際執行代碼或程式邏輯**。

### 2. 語法與編譯風險 (C# XML Doc Placement)
- **Info**: 所有的 XML `/// <summary>` 註解均正確放置在 Class, Interface, Method, Field, Property 的宣告上方。沒有將 XML 註解夾在 Attribute 之後或方法體內部的錯誤。

### 3. 語言與實質說明品質 (Comment Quality)
- **Info**: 註解皆採用清晰、專業的**繁體中文**撰寫。
- **Info**: 註解內容具有極高價值，不只是單純重述程式碼，而是解釋了**設計意圖**。例如：
  - 解釋了為何 `ToolUtility` / `PushUtility` 中要保留舊版 `AddRichMenuMessage` 的建構注入（為了相容性與測試邊界），但新流程改走 `assignment workflow`。
  - 解釋了 `InMemoryLineRichMenuIdCache` 的 `_gate` 鎖定設計與整份 Dictionary 取代的目的（防止讀取端讀到不完整的快取）。
  - 詳細記錄了 LINE API 的限制（例如：`userIds` 最多 500 筆、batch operation 最多 30 或 1000 筆、圖片必須為指定尺寸等）。

### 4. 覆蓋範圍評估 (Coverage check)
- **Info**: 異動涵蓋了 `Line.Messaging` 核心 SDK、`LineMessagingProcessor` 封裝層、`LineMessagingProcessor.AspNetCore` 註冊層、`RichMenus` 共用元件，以及 `ChurchReport` 產品實作與對應的單元測試。所有涉及 RichMenu 關鍵字的 cs 檔案均已補上註解，無遺漏。

### 5. 格式、編碼與 CRLF 驗證 (Format and Line Endings)
- **Info**: 原本被亂碼污染的註解（例如 `???`）已成功修復為正確的中文說明（如 `釋放 ToolUtilityClass`）。
- **Info**: 檔案編碼均維持 UTF-8 without BOM，且換行字元皆為 `CRLF`（\r\n），無格式混亂風險。

本批註解變更品質優良，對架構設計與平台限制有深度闡述，建議准予合併。

---
SESSION_ID: a5b773d9-84a3-4ce3-88a2-7c2127f86f45
